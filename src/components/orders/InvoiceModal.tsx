...
export default function InvoiceModal({ order, onComplete, onClose }: InvoiceModalProps) {
  ...
  const handlePrint = () => {
    ...
    // Siapkan konten invoice (sama seperti sebelumnya)
    ...
    const invoiceHtml = `
      ...
      <html>
        <head>
          ...
          <style>
            /* Salin semua style dari printWindow sebelumnya (saya singkat tapi lengkap) */
    ...
    // Tunggu iframe selesai memuat konten, lalu panggil print
    iframe.onload = () => {
      iframe.contentWindow?.focus();
      iframe.contentWindow?.print();

      // Hapus iframe setelah print dialog ditutup (perkiraan 1 detik, tidak ada event pasti)
      setTimeout(() => {
        document.body.removeChild(iframe);
      }, 1000);
    };

    // Jika onload tidak terpanggil (misal konten sudah siap), panggil manual
  ...
  const updateEditItem = (index: number, field: string, value: any) => {
    ...
    // Recalculate tax, discount, etc based on percentages
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const ownerId = orderDetails.user_id;

      // 1. Update order_items (delete dan insert ulang)
      ...
      // 2. Update orders table (termasuk customer fields)
      ...
      // 3. Update transactions jika ada
          ...
          {isEditing ? (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                ...
                <div>
                  <label className="block text-sm text-gray-600">Nama Customer</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm text-gray-600">Alamat Pengiriman</label>
                  <textarea
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span>Rp {editSubtotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Diskon (%)</span>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setEditDiscountAmount((editSubtotal * val) / 100);
                    }}
                    className="w-24 border rounded px-2 py-1 text-right"
                  />
                  <span>- Rp {editDiscountAmount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Pajak (%)</span>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setEditTaxAmount((editSubtotal * val) / 100);
                    }}
                    className="w-24 border rounded px-2 py-1 text-right"
                  />
                  <span>Rp {editTaxAmount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Service Charge (%)</span>
                  <input
                    type="number"
                    value={editServiceChargePercentage}
                    onChange={(e) => {
                      const val = Number(e.target.value);
                      setEditServiceChargePercentage(val);
                      setEditServiceChargeAmount((editSubtotal * val) / 100);
                    }}
                    className="w-24 border rounded px-2 py-1 text-right"
                  />
                  <span>Rp {editServiceChargeAmount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between font-bold">
                  <span>Total</span>
                  <span>
                    Rp {(editSubtotal + editTaxAmount + editServiceChargeAmount + editDeliveryFee - editDiscountAmount).toLocaleString()}
                  </span>
                </div>
                <div>
                  <label>Catatan</label>
            ...
            // Tampilkan invoice biasa (yang sudah ada)
